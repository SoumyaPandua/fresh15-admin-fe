...
import {
  ...
  BarChart3, ScrollText, UserCircle2, Leaf, Boxes, Star, BellRing, CalendarClock, Gift, FileCheck2, UploadCloud, BrainCircuit,
} from "lucide-react";
...
const groups: { label: string; items: { title: string; url: string; icon: any }[] }[] = [
  ...
  { label: "Insights", items: [
    ...
    { title: "Product Intelligence", url: "/product-intelligence", icon: BrainCircuit },
    ...
    <Sidebar collapsible="icon" className="border-r">
      <SidebarHeader className="border-b">
        <Link to="/" className="flex items-center gap-2.5 px-2 py-3">
          <div className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-primary text-primary-foreground shadow-sm"><Leaf className="h-5 w-5" /></div>
          <div className="min-w-0 group-data-[collapsible=icon]:hidden"><div className="truncate text-sm font-semibold tracking-tight">Fresh15</div><div className="truncate text-[11px] text-muted-foreground">Admin Console</div></div>
        </Link>
      </SidebarHeader>
      ...
      <SidebarFooter className="border-t">
        <Link to="/profile" className="flex items-center gap-2 rounded-md px-2 py-2 transition-colors hover:bg-muted group-data-[collapsible=icon]:hidden">
          ...
          <div className="min-w-0 flex-1"><div className="truncate text-xs font-medium">{user?.name || "Admin"}</div><div className="truncate text-[11px] text-muted-foreground">{user?.role || "Super Admin"}</div></div>
        </Link>
      </SidebarFooter>
    </Sidebar>
  );
