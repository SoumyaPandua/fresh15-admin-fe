...
export function LiveDeliveryMap({
  ...
  const trackingEnded = /delivered|cancelled|rejected/i.test(orderStatus ?? "");
  ...
  useEffect(() => {
    if (trackingEnded || !deliveryId || !token || !validCoordinate(current)) {
      return;
    }
