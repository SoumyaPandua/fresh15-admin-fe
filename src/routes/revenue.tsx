...
function RevenuePage() {
  ...
  if (error || !data) {
    return (
      <div className="space-y-6">
        <PageHeader title="Revenue" description="Track gross, net, refunds and revenue by category." />
        <Card className="p-5">
          <div className="text-sm font-semibold">Unable to load revenue</div>
  ...
  const { overview, revenueSeries, categoryMix } = data;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Gross revenue" value={inr(overview.grossRevenue)} delta={12.4} icon={IndianRupee} tone="success" />
        <StatCard label="Net revenue" value={inr(overview.netRevenue)} delta={11.6} icon={TrendingUp} tone="info" />
        <StatCard label="Refunds" value={inr(overview.refunds)} delta={-3.1} icon={Percent} tone="warning" />
        <StatCard label="AOV" value={inr(overview.aov)} delta={2.4} icon={ShoppingBag} />
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5 gap-4">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={categoryMix} innerRadius={60} outerRadius={100} paddingAngle={2} dataKey="value" stroke="none">
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={revenueSeries}>
                ...
                <YAxis tick={{ fontSize: 10, fill: "var(--muted-foreground)" }} axisLine={false} axisLine={false} tickLine={false} width={28} />
                <Tooltip contentStyle={{ background: "var(--popover)", border: "1px solid var(--border)", borderRadius: 12, fontSize: 12 }} />
