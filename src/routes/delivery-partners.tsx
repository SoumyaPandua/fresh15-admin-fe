...
function PartnersPage() {
  ...
  if (!admin) {
    return (
      <EmptyState
        icon={ShieldAlert}
        title="Partners are admin-only"
        description="Your account doesn't have permission to view delivery partners."
      />
    );
  }
  ...
  const columns: Column<PartnerRow>[] = [
    ...
    },
    {
      key: "email",
      header: "Email",
      render: (p) => <span className="text-sm">{p.rider.email ?? "—"}</span>,
    },
    {
      key: "presence",
      header: "Availability",
      render: (p) => {
        ...
        return pr ? (
          <StatusBadge
            label={pr.online ? "online" : "offline"}
            tone={pr.online ? "success" : "neutral"}
          />
        ) : (
    ...
    },
    {
      ...
      header: "Account",
      render: (p) =>
        p.rider.isActive === undefined ? (
          <span className="text-xs text-muted-foreground">—</span>
        ) : (
          <StatusBadge
            label={p.rider.isActive ? "active" : "inactive"}
            tone={p.rider.isActive ? "success" : "neutral"}
          />
        ),
    },
    {
      ...
      render: (p) =>
        p.current ? (
          <div className="flex flex-col gap-1">
            <StatusBadge
              label={deliveryLabel(p.current.status)}
              tone={deliveryTone(p.current.status) as any}
            />
            {p.current.orderLabel && (
    ...
    },
    {
      key: "completed",
      header: "Completed",
      accessor: (p) => p.completed,
      sortable: true,
      render: (p) => <span className="number text-sm">{p.completed}</span>,
    },
    {
      key: "earnings",
      header: "Earnings",
      accessor: (p) => p.earnings,
      sortable: true,
      render: (p) => <span className="number text-sm">{inr(p.earnings)}</span>,
    },
  ];
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <DataTable
          ...
          rowId={(p) => p.rider._id!}
          searchable={(p) =>
            `${p.rider.name ?? ""} ${p.rider.phone ?? ""} ${p.rider.email ?? ""}`
          }
